defmodule Teller.Transactions do
  ...
  @doc """
    ...
    The transaction id is generated by base64 encoding account_number and the sequence number of the transaction in the feed.
    This makes it easier to get the account_id of a transaction.
    The algorithm is such that it generates the transactions from the fixed date ~D[2021-09-19] to the current date and then
    takes the last 90 transactions.
    I have taken a fixed start date cause it becomes easy to maintain a consistent running balance even as the days progress.
    It is constant for all the accounts as of now but can be changed by encoding start date into the api token.
  """
  ...
  @doc """
    ...
    The transaction id is basically a base64 encoded form of account_id and the transaction's sequence number.
